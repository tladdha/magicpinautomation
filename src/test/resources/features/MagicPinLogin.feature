Feature: Magic Pin Login
  ...
  Scenario Outline: Login_TC_001 Verify the error for password miss-match while entering invalid credentials
  ...
  Scenario Outline: Login_TC_002 Verify the validations to email and password fields
    Given Launch the admin portal
    When Entering valid "<userName>" and "<Passwords>" credentials
    And Tap on Login
  ...
  Scenario Outline: Login_TC_003 Verify Forgot Password flow email validations
    Given Launch the admin portal
  ...
  Scenario Outline: Login_TC_004 Verify Forgot Password flow - Invalid user
    Given Launch the admin portal
    When Tap on forgot password hyperlink
    And Entering email id "<emails>"
    And Tap on Reset Password Button
  ...
  Scenario Outline: Login_TC_005 Verify Forgot Password flow - Valid user
    Given Launch the admin portal
    When Tap on forgot password hyperlink
    And Entering email id "<emails>"
    And Tap on Reset Password Button
